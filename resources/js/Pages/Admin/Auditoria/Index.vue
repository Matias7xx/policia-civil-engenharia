            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                ...
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg mb-6">
                    <div class="p-6">
                        ...
                        <form @submit.prevent="aplicarFiltros" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-7 gap-4">
                            <div>
                                ...
                                >
                                    ...
                                    <option v-for="usuario in usuarios" :key="usuario.id" :value="usuario.id">
                                        {{ usuario.name }} ({{ usuario.matricula }})
                                    </option>
                                </select>
                            </div>
                            ...
                            <div>
                                ...
                                >
                                    ...
                                    <option v-for="(label, value) in modeloTipos" :key="value" :value="value">
                                        {{ label }}
                                    </option>
                                </select>
                            </div>
                    ...
                    <div class="p-6">
                        <div class="flex items-center justify-between mb-6">
                            <h3 class="text-lg font-medium text-gray-900">
                                Histórico de Ações ({{ auditorias.total }} registros)
                            </h3>
                        </div>
                        ...
                        <div class="space-y-4">
                                ...
                                <div class="flex items-start justify-between">
                                    <div class="flex-1">
                                        ...
                                        <div class="flex items-center space-x-3 mb-2">
                                            ...
                                            <span class="text-sm font-medium text-gray-900">
                                                {{ auditoria.unidade_nome || 'Unidade não identificada' }}
                                            </span>
                                            <span v-if="auditoria.unidade_id" class="text-xs text-gray-500">
                                        ...
                                        <!-- Alterações -->
                                        <div v-if="auditoria.alteracoes.length > 0" class="bg-gray-50 rounded-lg p-3 mb-3">
                                            <h4 class="text-sm font-medium text-gray-700 mb-2">Alterações realizadas:</h4>
                                            <div class="space-y-2">
                                                <div 
                                                    v-for="alteracao in auditoria.alteracoes.slice(0, 3)" 
                                                    :key="alteracao.campo"
                                                    class="text-sm"
                                                >
                                                    <span class="font-medium text-gray-800">{{ alteracao.campo }}:</span>
                                                    <div class="ml-4 grid grid-cols-1 md:grid-cols-2 gap-2 mt-1">
                                                        <div v-if="alteracao.valor_antigo !== null" class="text-red-600">
                                                            <span class="font-medium">Antes:</span> 
                                                            <span class="bg-red-100 px-2 py-1 rounded text-xs">
                                                                {{ alteracao.valor_antigo || 'Não Informado' }}
                                                            </span>
                                                        </div>
                                                        <div class="text-green-600">
                                                            <span class="font-medium">{{ alteracao.valor_antigo !== null ? 'Depois' : '' }}</span> 
                                                            <span class="bg-green-100 px-2 py-1 rounded text-xs">
                                                                {{ alteracao.valor_novo || 'Não Informado' }}
                                                            </span>
                                                        </div>
                                                    </div>
                                                </div>
                                                <div v-if="auditoria.alteracoes.length > 3" class="text-xs text-gray-500">
                                                    + {{ auditoria.alteracoes.length - 3 }} alterações adicionais
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div class="flex items-center space-x-2">
                                        <Link 
                                            ...
                                            :href="route('admin.auditoria.show', auditoria.unidade_id)"
                            ...
                            <!-- Estado vazio -->
                            <div v-if="auditorias.data.length === 0" class="text-center py-12">
                                ...
                                <p class="mt-1 text-sm text-gray-500">
                                    Não há registros de auditoria com os filtros aplicados.
                                </p>
                            </div>
                        </div>

                        <!-- Paginação -->
                        <div v-if="auditorias.links && auditorias.data.length > 0" class="mt-6">
                                ...
                                <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                                    <div>
                                        <p class="text-sm text-gray-700">
                                            Mostrando
                                            <span class="font-medium">{{ auditorias.from }}</span>
                                            a
                                            <span class="font-medium">{{ auditorias.to }}</span>
                                            de
                                    ...
                                    <div>
                                        <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                                            ...
                                            <template v-for="link in auditorias.links" :key="link.label">
                                                <Link 
