    ...
    <AppLayout title="Detalhes da Auditoria">
        <div class="py-12">
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
                        ...
                        <div class="flex items-center justify-between">
                            <div>
                                <h1 class="text-2xl font-bold text-gray-900 flex items-center">
                                    <DocumentMagnifyingGlassIcon class="h-8 w-8 mr-3 text-[#bea55a]" />
                                ...
                                <div class="mt-2 flex items-center space-x-4 text-sm text-gray-500">
                                    ...
                                    <span>Código: {{ unidade.codigo || 'N/A' }}</span>
                                    <span>Cidade: {{ unidade.cidade || 'N/A' }}</span>
                                </div>
                            </div>
                ...
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg mb-6">
                    <div class="p-6">
                        <h3 class="text-lg font-medium text-gray-900 mb-4">Filtrar por Tipo de Alteração</h3>
                ...
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg">
                    <div class="p-6">
                        <h3 class="text-lg font-medium text-gray-900 mb-6">
                            Histórico Detalhado
                            <span v-if="auditoriasFiltradas.length !== auditorias.length" class="text-sm text-gray-500">
                                ({{ auditoriasFiltradas.length }} de {{ auditorias.length }} registros)
                            </span>
                        </h3>

                        <div class="flow-root">
                            <ul role="list" class="-mb-8">
                                <li v-for="(auditoria, index) in auditoriasFiltradas" :key="auditoria.id" class="relative pb-8">
                                    <!-- Linha conectora -->
                                    <span 
                                        v-if="index !== auditoriasFiltradas.length - 1" 
                                        ...
                                        <div class="relative">
                                            <span 
                                                :class="[
                                                    ...
                                                    getEventoIconClass(auditoria.evento)
                                                ]"
                                            >
                                                <component :is="getEventoIcon(auditoria.evento)" class="h-5 w-5" />
                                            </span>
                                        </div>
                                            ...
                                            <div class="bg-gray-50 rounded-lg p-4">
                                                ...
                                                <div class="flex items-center justify-between mb-3">
                                                    <div class="flex items-center space-x-3">
                                                        <span 
                                                            :class="getEventoClass(auditoria.evento)"
                                                        ...
                                                        <span v-if="auditoria.descricao_item" class="text-sm text-gray-600">
                                                ...
                                                <div class="flex items-center space-x-4 text-sm text-gray-600 mb-3">
                                                    <div class="flex items-center">
                                                        ...
                                                        {{ auditoria.usuario }}
                                                        <span v-if="auditoria.usuario_matricula" class="ml-1 text-xs text-gray-500">
                                                            ({{ auditoria.usuario_matricula }})
                                                        </span>
                                                    </div>
                                                    <div v-if="auditoria.ip_address" class="flex items-center">
                                                ...
                                                <!-- Alterações -->
                                                <div v-if="auditoria.alteracoes.length > 0" class="mb-3">
                                                    ...
                                                    <div class="space-y-3">
                                                        ...
                                                        >
                                                            ...
                                                            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                                                                <div v-if="alteracao.valor_antigo !== null" class="text-red-600">
                                                                    <div class="text-xs font-medium mb-1">Valor Anterior:</div>
                                                                    <div class="bg-red-50 border border-red-200 rounded px-2 py-1 text-sm">
                                                                        {{ formatarValor(alteracao.valor_antigo) }}
                                                                    </div>
                                                                </div>
                                                                <div class="text-green-600">
                                                                    <div class="text-xs font-medium mb-1">
                                                                        {{ alteracao.valor_antigo !== null ? 'Novo Valor' : 'Valor' }}:
                                                                    </div>
                                                                    <div class="bg-green-50 border border-green-200 rounded px-2 py-1 text-sm">
                                                                        {{ formatarValor(alteracao.valor_novo) }}
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </div>
                                                ...
                                                <div class="border-t border-gray-200 pt-3">
                                                    <button 
                                                        @click="toggleDetalhes(auditoria.id)"
                                                        class="flex items-center text-xs text-gray-500 hover:text-gray-700"
                                                    >
                                                        <ChevronDownIcon 
                                                            :class="[
                                                                'h-4 w-4 mr-1 transition-transform',
                                                                detalhesExpandidos[auditoria.id] ? 'rotate-180' : ''
                                                            ]"
                                                        />
                                                        {{ detalhesExpandidos[auditoria.id] ? 'Ocultar' : 'Ver' }} detalhes técnicos
                                                    </button>
                                                    
                                                    <div v-if="detalhesExpandidos[auditoria.id]" class="mt-2 text-xs text-gray-600 space-y-1">
                                                        <div><strong>ID da Auditoria:</strong> {{ auditoria.id }}</div>
                                                        <div><strong>Modelo:</strong> {{ auditoria.modelo_tipo }} (ID: {{ auditoria.modelo_id }})</div>
                                                        <div v-if="auditoria.url"><strong>URL:</strong> {{ auditoria.url }}</div>
                                                        <div v-if="auditoria.user_agent"><strong>User Agent:</strong> {{ auditoria.user_agent }}</div>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                        ...
                        <div v-if="auditoriasFiltradas.length === 0" class="text-center py-12">
                            ...
                            <p class="mt-1 text-sm text-gray-500">
                                Não há registros de auditoria para o filtro selecionado.
                            </p>
                            <button 
                                @click="filtroTipo = null"
...
const auditoriasFiltradas = computed(() => {
    ...
    return props.auditorias.filter(auditoria => auditoria.modelo_tipo === filtroTipo.value);
});
...
const contarEventos = (evento) => {
    return props.auditorias.filter(auditoria => auditoria.evento === evento).length;
};
