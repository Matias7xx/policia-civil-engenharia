...
const getTrendIconClass = (trend) => {
  switch(trend) {
    case 'positivo': return 'text-green-500 transform rotate-180';
...
const getTrendText = (field) => {
  ...
  switch(trend) {
    case 'positivo': return `+${diff} desde última avaliação`;
    case 'negativo': return `-${diff} desde última avaliação`;
...
<template>
  <div class="bg-white rounded-lg shadow-md overflow-hidden">
    ...
    <div v-if="!avaliacoes || avaliacoes.length === 0" class="text-center py-10 px-6 text-gray-500">
      ...
      <p class="mt-2 text-sm">
        <span v-if="isSuperAdmin">Clique em "Nova Avaliação" para realizar a primeira avaliação.</span>
        <span v-else>Aguarde que um administrador realize a avaliação da unidade.</span>
      </p>
    </div>
      ...
      >
        <div class="flex flex-col sm:flex-row sm:items-center justify-between">
          ...
          <div class="flex items-center">
            <div class="mr-4 flex-shrink-0 size-10 rounded-full bg-[#bea55a] bg-opacity-20 flex items-center justify-center">
          ...
          <div class="flex flex-wrap gap-2 mt-3 sm:mt-0">
            <div class="p-1 bg-gray-100 rounded-full flex items-center">
              <span class="text-xs text-gray-500 ml-1 mr-2">Geral:</span>
              <span :class="getNotaClass(avaliacao.nota_geral)" class="px-2 py-0.5 rounded-full text-xs font-medium">
                {{ avaliacao.nota_geral }} {{ getNotaLetra(avaliacao.nota_geral) }}
              </span>
            </div>
          </div>
        </div>
    ...
    <!-- Modal de detalhes -->
    <div v-if="showDetails && selectedAvaliacao" class="fixed inset-0 bg-black bg-opacity-50 z-[1000] flex items-center justify-center p-4">
        ...
        <div class="p-6">
          ...
          <div class="mb-6 bg-gray-50 p-4 rounded-lg">
            <div class="flex items-center justify-between mb-3">
              <div class="text-sm text-gray-500">
                Unidade avaliada em: {{ formatDate(selectedAvaliacao.created_at) }}
              </div>
            </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
            <div class="p-3 border rounded-lg">
              <div class="text-xs text-gray-500 mb-1">Nota Geral</div>
              <div class="flex items-center">
                <span class="text-2xl font-bold mr-2">{{ selectedAvaliacao.nota_geral }}</span>
                <span :class="getNotaClass(selectedAvaliacao.nota_geral)" class="text-xs font-medium p-1 px-2 rounded">
                  {{ getNotaLetra(selectedAvaliacao.nota_geral) }}
                </span>
              </div>
            </div>
            <div class="p-3 border rounded-lg">
              <div class="text-xs text-gray-500 mb-1">Nota Estrutura</div>
              <div class="flex items-center">
                <span class="text-2xl font-bold mr-2">{{ selectedAvaliacao.nota_estrutura }}</span>
                <span :class="getNotaClass(selectedAvaliacao.nota_estrutura)" class="text-xs font-medium p-1 px-2 rounded">
                  {{ getNotaLetra(selectedAvaliacao.nota_estrutura) }}
                </span>
              </div>
            </div>
            <div class="p-3 border rounded-lg">
              <div class="text-xs text-gray-500 mb-1">Nota Acessibilidade</div>
              <div class="flex items-center">
                <span class="text-2xl font-bold mr-2">{{ selectedAvaliacao.nota_acessibilidade }}</span>
                <span :class="getNotaClass(selectedAvaliacao.nota_acessibilidade)" class="text-xs font-medium p-1 px-2 rounded">
                  {{ getNotaLetra(selectedAvaliacao.nota_acessibilidade) }}
                </span>
              </div>
            </div>
          </div>
          ...
          <div v-if="selectedAvaliacao.observacoes" class="mb-6">
            <h4 class="text-sm font-medium text-gray-700 mb-2">Observações e Recomendações:</h4>
            <div class="p-4 bg-gray-50 rounded-lg border text-sm text-gray-600 whitespace-pre-line">
