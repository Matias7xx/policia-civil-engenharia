...
const ambientesNaoPossui = computed(() => {
    ...
    return props.midias
        ...
        .map((midia) => {
            ...
            const nome =
                midia.midia_tipo?.nome ||
                midia.midiaTipo?.nome ||
...
const contratoForm = useForm({
    ...
    data_inicio: formatarDataParaInput(
        props.unidade?.contrato_locacao?.data_inicio,
    ),
...
const telefoneMask = {
    ...
    dispatch: function (appended, dynamicMasked) {
        const numericValue = (dynamicMasked.value + appended).replace(
            /\D/g,
            "",
        );
...
watch(
    ...
    (flash) => {
        flashMessage.value =
            flash?.success || flash?.error || flash?.banner || null;
...
const submitStatus = () => {
    if (statusForm.status === "reprovada") {
        ...
        if (!statusForm.rejection_reason) {
            flashMessage.value =
                "Por favor, forneça um motivo para a reprovação.";
            setTimeout(() => (flashMessage.value = null), 5000);
            return;
        }
    }
    statusForm.post(route("admin.unidades.updateStatus", props.unidade.id), {
        ...
        onError: (errors) => {
            flashMessage.value =
                ...
                (errors.rejection_reason || "Verifique os dados.");
            setTimeout(() => (flashMessage.value = null), 5000);
        },
    });
};
...
const salvarContrato = () => {
    ...
    const cleanedCpfCnpj = contratoForm.cpf_cnpj
        ? String(contratoForm.cpf_cnpj).replace(/\D/g, "").trim()
        : "";
    const cleanedTelefone = contratoForm.telefone
        ? String(contratoForm.telefone).replace(/\D/g, "").trim()
        : null;
    ...
    contratoForm.post(
        route("admin.unidades.updateContrato", props.unidade.id),
        {
            preserveState: true,
            preserveScroll: true,
            forceFormData: true,
            onSuccess: (page) => {
                ...
                const updatedContrato =
                    page.props.unidade.contrato_locacao || {};
                contratoForm.nome_proprietario =
                    updatedContrato.nome_proprietario || "";
                contratoForm.cpf_cnpj = updatedContrato.cpf_cnpj || "";
                contratoForm.telefone = updatedContrato.telefone || "";
                contratoForm.valor_locacao =
                    updatedContrato.valor_locacao || "";
            ...
            onError: () => {
                toast.error(
                    "Erro ao salvar os dados de contrato. Verifique os campos.",
                );
                flashMessage.value =
                    "Erro ao salvar o contrato. Verifique os campos.";
                setTimeout(() => (flashMessage.value = null), 5000);
            },
        },
    );
...
const salvarCessao = () => {
    cessaoForm.post(route("admin.unidades.updateCessao", props.unidade.id), {
        ...
        onError: () => {
            toast.error(
                "Erro ao salvar os dados de cessão. Verifique os campos.",
            );
            flashMessage.value =
                "Erro ao salvar os dados de cessão. Verifique os campos.";
            setTimeout(() => (flashMessage.value = null), 5000);
...
<template>
    <AppLayout
        :title="`Visualizar Unidade: ${unidade?.nome || 'Carregando...'}`"
    >
        <template #header>
        ...
        <div
            v-if="flashMessage"
            class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 my-4"
        >
            <div
                class="p-4 rounded-md shadow-sm transition-all"
                :class="
                    flashMessage.includes('sucesso') ||
                    flashMessage.includes('salva')
                        ...
                        <i
                            ...
                            :class="
                                ...
                                flashMessage.includes('salva')
                                    ...
                                    : 'fa-exclamation-circle text-amber-600'
                            "
                        ></i>
                    </div>
        ...
        <div
            v-if="page.props.errors?.unauthorized"
            class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 my-4"
        >
            <div
                class="p-4 rounded-md bg-red-100 text-red-700 border-l-4 border-red-500"
            >
                <div class="flex">
                    <div class="flex-shrink-0">
            ...
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <div class="bg-white rounded-t-lg overflow-hidden shadow-md">
                    <div class="hidden sm:block border-b border-gray-200">
                        <nav
                            class="flex space-x-2 md:space-x-8 overflow-x-auto scrollbar-thin"
                            aria-label="Abas"
                        >
                            <button
                                v-for="tab in tabs"
                    ...
                    <!-- Navegação mobile -->
                    <div
                        class="sm:hidden bg-white p-4 border-b border-gray-200"
                    >
                        <div class="flex items-center justify-between">
                            <span class="text-indigo-600 font-medium">
                                <i
                                    :class="`fas ${tabs.find((t) => t.id === activeTab)?.icon || 'fa-info-circle'} mr-2`"
                                ></i>
                                {{
                        ...
                        <div
                            v-if="mobileMenuOpen"
                            class="mt-2 space-y-1 bg-gray-50 rounded-md p-2 transition-all"
                        >
                            <button
                                v-for="tab in tabs"
                                :key="tab.id"
                    ...
                    <div class="p-4 sm:p-6 bg-white">
                        ...
                        <div v-if="unidade?.status" class="mb-6">
                            <div class="flex flex-wrap items-center gap-2">
                                <span class="font-medium text-gray-600"
                                    >Preenchimento do Formulário:</span
                                >
                                <span
                            ...
                            >
                                <div class="bg-red-50 p-3 rounded-md shadow-sm">
                                    ...
                                    <dd
                                        class="mt-1 whitespace-pre-line text-red-700"
                                    >
                                        {{ unidade.rejection_reason }}
                                    </dd>
                                </div>
                            ...
                            >
                                <div class="bg-red-50 p-3 rounded-md shadow-sm">
                                    <dt
                                        class="font-medium text-gray-600 text-xs uppercase tracking-wider"
                                    >
                                        O formulário enviado por esta unidade
                                        foi reprovado
                                    </dt>
                                    <dt
                                        class="font-medium text-gray-600 text-xs uppercase tracking-wider"
                                    >
                                        Motivo da última Reprovação:
                                    </dt>
                                    <dd
                                        class="mt-1 whitespace-pre-line text-red-700"
                                    >
                        ...
                        >
                            ...
                            <div
                                v-if="
                                    unidade?.tipo_judicial === 'locado' &&
                                    unidade?.contrato_locacao
                                "
                                class="bg-gray-50 p-4 rounded-lg shadow-sm md:col-span-2"
                            >
                                <h3
                                    class="text-lg font-medium text-gray-900 border-b pb-2 mb-4"
                                >
                                ...
                                >
                                    ...
                                    <div
                                        v-if="unidade.contrato_locacao?.anexo"
                                        class="bg-white p-3 rounded-md shadow-sm hover:shadow-md transition-shadow"
                                    >
                                        <dt
                                            class="font-medium text-gray-600 text-xs uppercase tracking-wider"
                            ...
                            >
                                ...
                                >
                                    ...
                                    <div>
                                        ...
                                        />
                                        <p
                                            v-if="contratoForm.errors.cpf_cnpj"
                                            class="text-red-500 text-xs mt-1"
                                        >
                                            {{ contratoForm.errors.cpf_cnpj }}
                                        </p>
                                    </div>
                                    <div>
                                        <label
                                        ...
                                        />
                                        <p
                                            v-if="contratoForm.errors.telefone"
                                            class="text-red-500 text-xs mt-1"
                                        >
                                            {{ contratoForm.errors.telefone }}
                                        </p>
                                    </div>
                                    <div>
                                        <label
                                            class="block text-sm font-medium text-gray-700"
                                            >Valor da Locação (R$)</label
                                        >
                                        <input
                                            v-model="contratoForm.valor_locacao"
                                            type="number"
                                            step="0.01"
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm"
                                        />
                                    ...
                                    <div>
                                        ...
                                        <p
                                            v-if="
                                                contratoForm.errors.data_inicio
                                            "
                                            class="text-red-500 text-xs mt-1"
                                        >
                                            {{
                                    ...
                                    <div>
                                        ...
                                        />
                                        <p
                                            v-if="contratoForm.errors.data_fim"
                                            class="text-red-500 text-xs mt-1"
                                        >
                                            {{ contratoForm.errors.data_fim }}
                                        </p>
                                    </div>
                                    <div>
                                        <label
                                        ...
                                        />
                                        <p
                                            v-if="contratoForm.errors.anexo"
                                            class="text-red-500 text-xs mt-1"
                                        >
                                            {{ contratoForm.errors.anexo }}
                                        </p>
                                    </div>
                            ...
                            <div
                                v-if="unidade?.tipo_judicial === 'cedido'"
                                class="bg-gray-50 p-4 rounded-lg shadow-sm md:col-span-2"
                            >
                                <h3
                                    class="text-lg font-medium text-gray-900 border-b pb-2 mb-4"
                                >
                                    Informações de Cessão
                                </h3>
                                <div
                                    class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4"
                                >
                                    ...
                                    >
                                        ...
                                        <dd class="mt-1">
                                            {{
                                                formatarData(
                                                    unidade?.prazo_cessao,
                                                )
                                            }}
                                        </dd>
                                    </div>
                                    <div
                                        v-if="unidade.termo_cessao"
                                        class="bg-white p-3 rounded-md shadow-sm hover:shadow-md transition-shadow"
                                    >
                                        <dt
                                            class="font-medium text-gray-600 text-xs uppercase tracking-wider"
                            ...
                            >
                                <h3
                                    class="text-lg font-medium text-gray-900 border-b pb-2 mb-4"
                                >
                                    Editar Informações de Cessão
                                </h3>
                                <form
                                    @submit.prevent="salvarCessao"
                                    class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4"
                                >
                                    <div>
                                        <label
                                        ...
                                        >
                                        <input
                                            v-model="cessaoForm.orgao_cedente"
                                            type="text"
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm"
                                            required
                                        />
                                        <p
                                            v-if="
                                                cessaoForm.errors.orgao_cedente
                                            "
                                            class="text-red-500 text-xs mt-1"
                                        >
                                            {{
                                    ...
                                    <div>
                                        <label
                                            class="block text-sm font-medium text-gray-700"
                                            >Prazo de Cessão</label
                                        >
                                        <input
                                            v-model="cessaoForm.prazo_cessao"
                                            type="date"
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm"
                                            required
                                        />
                                        <p
                                            v-if="
                                                cessaoForm.errors.prazo_cessao
                                            "
                                            class="text-red-500 text-xs mt-1"
                                        >
                                            {{ cessaoForm.errors.prazo_cessao }}
                                    ...
                                    <div>
                                        <label
                                            class="block text-sm font-medium text-gray-700"
                                            >Termo de Cessão</label
                                        >
                                        <input
                                            type="file"
                                            @change="
                                                cessaoForm.termo_cessao =
                                                    $event.target.files[0]
                                            "
                                            accept="application/pdf"
                                            class="mt-1 block w-full rounded-md border-gray-300 shadow-sm"
                                        ...
                                        <p
                                            v-if="
                                                cessaoForm.errors.termo_cessao
                                            "
                                            class="text-red-500 text-xs mt-1"
                                        >
                                            {{ cessaoForm.errors.termo_cessao }}
                            ...
                            <div
                                v-if="unidade?.imovel_compartilhado_unidades"
                                class="bg-gray-50 p-4 rounded-lg shadow-sm md:col-span-2"
                            >
                                <h3
                                    class="text-lg font-medium text-gray-900 border-b pb-2 mb-4"
                                >
                                    Compartilhamento com Unidades Policiais
                                </h3>

                                <!-- Unidades selecionadas via dropdown -->
                                <div
                                    v-if="
                                        unidade.unidadesCompartilhadas &&
                                        unidade.unidadesCompartilhadas.length >
                                            0
                                    "
                                    class="bg-white p-3 rounded-md shadow-sm hover:shadow-md transition-shadow mb-4"
                                >
                                    <dt
                                        class="font-medium text-gray-600 text-xs uppercase tracking-wider"
                                    >
                                        Unidades Policiais que compartilham o
                                        imóvel:
                                    </dt>
                                ...
                                <div
                                    v-if="
                                        unidade.imovel_compartilhado_unidades_texto?.trim()
                                    "
                                    class="bg-white p-3 rounded-md shadow-sm hover:shadow-md transition-shadow"
                                >
                                    <dt
                                        class="font-medium text-gray-600 text-xs uppercase tracking-wider"
                                    >
                                        Descrição das unidades que compartilham
                                        o imóvel:
                                    </dt>
                                ...
                                >
                                    <dt
                                        class="font-medium text-gray-600 text-xs uppercase tracking-wider"
                                    >
                                        Unidades que compartilham o imóvel:
                                    </dt>
                            ...
                            <div
                                v-if="unidade?.imovel_compartilhado_orgao"
                                class="bg-gray-50 p-4 rounded-lg shadow-sm md:col-span-2"
                            >
                                <h3
                                    class="text-lg font-medium text-gray-900 border-b pb-2 mb-4"
                                ...
                                >
                                    <dt
                                        class="font-medium text-gray-600 text-xs uppercase tracking-wider"
                                    >
                                        Órgãos que compartilham o imóvel:
                                    </dt>
                                    <dd class="mt-1 flex flex-wrap gap-2">
                                        <span
                                            v-if="
                                                !unidade.orgaosCompartilhados ||
                                                unidade.orgaosCompartilhados
                                                    .length === 0
                                            "
                                            class="text-gray-600"
                            ...
                            >
                                ...
                                <div
                                    class="bg-white p-3 rounded-md shadow-sm hover:shadow-md transition-shadow whitespace-pre-line"
                                >
                                    {{ unidade.observacoes }}
                                </div>
                            </div>
                        ...
                        >
                            ...
                            >
                                ...
                                <div
                                    v-if="!acessibilidade"
                                    class="bg-white p-6 rounded-md shadow-sm text-center"
                                >
                                    <i
                                        class="fas fa-info-circle text-blue-500 text-4xl mb-4"
                                    ></i>
                                    <p class="text-gray-600">
                                        Nenhuma informação de acessibilidade
                                        cadastrada para esta unidade.
                                    </p>
                                </div>

                                <!-- Observações de Acessibilidade -->
                                <div
                                    v-if="acessibilidade?.observacoes"
                                    class="bg-white p-3 rounded-md shadow-sm hover:shadow-md transition-shadow mt-4"
                                >
                                    <dt
                                        class="font-medium text-gray-600 text-xs uppercase tracking-wider"
                                    >
                        ...
                        >
                            ...
                            >
                                ...
                                <div
                                    class="bg-white p-3 rounded-md shadow-sm hover:shadow-md transition-shadow mt-4 w-fit"
                                >
                                    <dt
                                        class="font-medium text-gray-600 text-xs uppercase tracking-wider"
                                    >
                                        Ponto de energia próximo a algum ponto
                                        de água para a instalação de um
                                        purificador de água:
                                    </dt>
                                    <dd class="mt-1">
                                        {{
                                            informacoes.ponto_energia_agua ||
                                            "Não informado"
                                        }}
                                    </dd>
                                </div>
                                ...
                                >
                                    ...
                                    >
                                        <dt
                                            class="font-medium text-gray-600 text-xs uppercase tracking-wider"
                                        >
                                            Área Aproximada da Unidade (Área
                                            Construída) m²:
                                        </dt>
                                        <dd class="mt-1">
                                            {{
                                                informacoes.area_aproximada_unidade
                                                    ? `${informacoes.area_aproximada_unidade} m²`
                                                    : "Não informado"
                                            }}
                                        </dd>
                                    </div>
                                    ...
                                    >
                                        <dt
                                            class="font-medium text-gray-600 text-xs uppercase tracking-wider"
                                        >
                                            Área Aproximada do Terreno (m²):
                                        </dt>
                                        <dd class="mt-1">
                                            {{
                                                informacoes.area_aproximada_terreno
                                                    ? `${informacoes.area_aproximada_terreno} m²`
                                                    : "Não informado"
                                            }}
                                        </dd>
                                    </div>
                                    ...
                                    >
                                        ...
                                        <dd class="mt-1 flex items-center">
                                            <i
                                                :class="`fas ${informacoes.cercado_muros ? 'fa-check text-green-500' : 'fa-times text-red-500'} mr-2`"
                                            ></i>
                                            {{
                                                informacoes.cercado_muros
                                                    ? "Sim"
                                                    : "Não"
                                            }}
                                        </dd>
                                    ...
                                    >
                                        ...
                                        <dd class="mt-1 flex items-center">
                                            <i
                                                :class="`fas ${informacoes.estacionamento_interno ? 'fa-check text-green-500' : 'fa-times text-red-500'} mr-2`"
                                            ></i>
                                            {{
                                                informacoes.estacionamento_interno
                                                    ? "Sim"
                                                    : "Não"
                                            }}
                                        </dd>
                                    ...
                                    >
                                        ...
                                        <dd class="mt-1 flex items-center">
                                            <i
                                                :class="`fas ${informacoes.estacionamento_externo ? 'fa-check text-green-500' : 'fa-times text-red-500'} mr-2`"
                                            ></i>
                                            {{
                                                informacoes.estacionamento_externo
                                                    ? "Sim"
                                                    : "Não"
                                            }}
                                        </dd>
                            ...
                            >
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <dd class="mt-1">
                                            {{
                                                informacoes.recuo_frontal
                                                    ? `${informacoes.recuo_frontal} m`
                                                    : "Não informado"
                                            }}
                                        </dd>
                                    </div>
                                    ...
                                    >
                                        ...
                                        <dd class="mt-1">
                                            {{
                                                informacoes.recuo_lateral
                                                    ? `${informacoes.recuo_lateral} m`
                                                    : "Não informado"
                                            }}
                                        </dd>
                                    </div>
                                    ...
                                    >
                                        ...
                                        <dd class="mt-1">
                                            {{
                                                informacoes.recuo_fundos
                                                    ? `${informacoes.recuo_fundos} m`
                                                    : "Não informado"
                                            }}
                                        </dd>
                                    </div>
                                ...
                                >
                                    ...
                                    >
                                        <dt
                                            class="font-medium text-gray-600 text-xs uppercase tracking-wider"
                                        >
                                            Área Xadrez masculino:
                                        </dt>
                                        <dd class="mt-1">
                                            {{
                                                informacoes.area_xadrez_masculino
                                                    ? `${informacoes.area_xadrez_masculino} m²`
                                                    : "Não informado"
                                            }}
                                        </dd>
                                    </div>
                                    ...
                                    >
                                        <dt
                                            class="font-medium text-gray-600 text-xs uppercase tracking-wider"
                                        >
                                            Área Xadrez feminino:
                                        </dt>
                                        <dd class="mt-1">
                                            {{
                                                informacoes.area_xadrez_feminino
                                                    ? `${informacoes.area_xadrez_feminino} m²`
                                                    : "Não informado"
                                            }}
                                        </dd>
                                    </div>
                            ...
                            >
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        <dd class="mt-1 flex items-center">
                                            <i
                                                :class="`fas ${informacoes.tomadas_suficientes ? 'fa-check text-green-500' : 'fa-times text-red-500'} mr-2`"
                                            ></i>
                                            {{
                                                informacoes.tomadas_suficientes
                                                    ? "Suficientes"
                                                    : "Insuficientes"
                                            }}
                                        </dd>
                                    ...
                                    >
                                        ...
                                        <dd class="mt-1 flex items-center">
                                            <i
                                                :class="`fas ${informacoes.luminarias_suficientes ? 'fa-check text-green-500' : 'fa-times text-red-500'} mr-2`"
                                            ></i>
                                            {{
                                                informacoes.luminarias_suficientes
                                                    ? "Suficientes"
                                                    : "Insuficientes"
                                            }}
                                        </dd>
                                    ...
                                    >
                                        ...
                                        <dd class="mt-1 flex items-center">
                                            <i
                                                :class="`fas ${informacoes.pontos_rede_suficientes ? 'fa-check text-green-500' : 'fa-times text-red-500'} mr-2`"
                                            ></i>
                                            {{
                                                informacoes.pontos_rede_suficientes
                                                    ? "Suficientes"
                                                    : "Insuficientes"
                                            }}
                                        </dd>
                                    ...
                                    >
                                        ...
                                        <dd class="mt-1 flex items-center">
                                            <i
                                                :class="`fas ${informacoes.pontos_telefone_suficientes ? 'fa-check text-green-500' : 'fa-times text-red-500'} mr-2`"
                                            ></i>
                                            {{
                                                informacoes.pontos_telefone_suficientes
                                                    ? "Suficientes"
                                                    : "Insuficientes"
                                            }}
                                        </dd>
                                    ...
                                    >
                                        ...
                                        <dd class="mt-1 flex items-center">
                                            <i
                                                :class="`fas ${informacoes.pontos_ar_condicionado_suficientes ? 'fa-check text-green-500' : 'fa-times text-red-500'} mr-2`"
                                            ></i>
                                            {{
                                                informacoes.pontos_ar_condicionado_suficientes
                                                    ? "Suficientes"
                                                    : "Insuficientes"
                                            }}
                                        </dd>
                                    ...
                                    >
                                        ...
                                        <dd class="mt-1 flex items-center">
                                            <i
                                                :class="`fas ${informacoes.pontos_hidraulicos_suficientes ? 'fa-check text-green-500' : 'fa-times text-red-500'} mr-2`"
                                            ></i>
                                            {{
                                                informacoes.pontos_hidraulicos_suficientes
                                                    ? "Suficientes"
                                                    : "Insuficientes"
                                            }}
                                        </dd>
                                    ...
                                    >
                                        ...
                                        <dd class="mt-1 flex items-center">
                                            <i
                                                :class="`fas ${informacoes.pontos_sanitarios_suficientes ? 'fa-check text-green-500' : 'fa-times text-red-500'} mr-2`"
                                            ></i>
                                            {{
                                                informacoes.pontos_sanitarios_suficientes
                                                    ? "Suficientes"
                                                    : "Insuficientes"
                                            }}
                                        </dd>
                                ...
                                >
                                    ...
                                    >
                                        <dt
                                            class="font-medium text-gray-600 text-xs uppercase tracking-wider"
                                        >
                                            Demarcação no piso abaixo do
                                            extintor:
                                        </dt>
                                        <dd class="mt-1">
                                            {{
                                                informacoes.demarcacao_piso_extintor ||
                                                "Não informado"
                                            }}
                                        </dd>
                                    </div>
                                    ...
                                    >
                                        <dt
                                            class="font-medium text-gray-600 text-xs uppercase tracking-wider"
                                        >
                                            A porta principal abre para fora:
                                        </dt>
                                        <dd class="mt-1">
                                            {{
                                                informacoes.porta_principal_abre_fora ||
                                                "Não informado"
                                            }}
                                        </dd>
                                    </div>
                            ...
                            >
                                ...
                                >
                                    <dt
                                        class="font-medium text-gray-600 text-xs uppercase tracking-wider"
                                    >
                                        Na unidade policial tem espaço para
                                        guardar veículos apreendidos:
                                    </dt>
                                    <dd class="mt-1 flex items-center">
                                        <i
                                            :class="`fas ${informacoes.tem_espaco_veiculos_apreendidos ? 'fa-check text-green-500' : 'fa-times text-red-500'} mr-2`"
                                        ></i>
                                        {{
                                            informacoes.tem_espaco_veiculos_apreendidos
                                                ? "Sim"
                                                : "Não"
                                        }}
                                    </dd>
                                ...
                                >
                                    ...
                                    >
                                        <dt
                                            class="font-medium text-gray-600 text-xs uppercase tracking-wider"
                                        >
                                            Quantos veículos do tipo automóvel
                                            podem ser guardados:
                                        </dt>
                                        <dd class="mt-1 font-semibold text-lg">
                                            {{
                                                informacoes.qtd_max_veiculos_automovel
                                                    ? `${informacoes.qtd_max_veiculos_automovel} veículos`
                                                    : "Não informado"
                                            }}
                                        </dd>
                                    </div>
                                    ...
                                    >
                                        ...
                                        <dd class="mt-1 flex items-center">
                                            <span
                                                ...
                                                :class="{
                                                    'bg-green-100 text-green-800':
                                                        informacoes.seguranca_local_veiculos ===
                                                        'sim',
                                            ...
                                            >
                                                <i
                                                    :class="{
                                                        'fas fa-shield-alt mr-1':
                                                            informacoes.seguranca_local_veiculos ===
                                                            'sim',
                                                        'fas fa-exclamation-triangle mr-1':
                                                            informacoes.seguranca_local_veiculos ===
                                                            'nao',
                                                        'fas fa-shield-halved mr-1':
                                                            informacoes.seguranca_local_veiculos ===
                                                            'parcial',
                                                        'fas fa-question mr-1':
                                                            !informacoes.seguranca_local_veiculos,
                                                    }"
                                                ></i>
                                                {{
                                    ...
                                    >
                                        <dt
                                            class="font-medium text-gray-600 text-xs uppercase tracking-wider"
                                        >
                                            Histórico de invasão/subtração:
                                        </dt>
                                        <dd class="mt-1 flex items-center">
                                            <i
                                                :class="`fas ${informacoes.historico_invasao_veiculo ? 'fa-exclamation-triangle text-red-500' : 'fa-check text-green-500'} mr-2`"
                                            ></i>
                                            <span
                                                :class="
                                                    informacoes.historico_invasao_veiculo
                                    ...
                                    <div
                                        v-if="
                                            informacoes.observacoes_veiculos_apreendidos
                                        "
                                        class="bg-white p-3 rounded-md shadow-sm hover:shadow-md transition-shadow sm:col-span-2 md:col-span-3"
                                    >
                                        <dt
                                            class="font-medium text-gray-600 text-xs uppercase tracking-wider"
                                        >
                                        ...
                                        <dd
                                            class="mt-1 whitespace-pre-line text-gray-700"
                                        >
                                            {{
                                                informacoes.observacoes_veiculos_apreendidos
                                            }}
                                ...
                                <div
                                    v-else
                                    class="bg-yellow-50 p-3 rounded-md border border-yellow-200"
                                >
                                    <div class="flex items-center">
                                        <i
                                            class="fas fa-info-circle text-yellow-600 mr-2"
                                        ></i>
                                        <p class="text-yellow-800 text-sm">
                                            Esta unidade não possui espaço
                                            disponível para guardar veículos
                                            apreendidos.
                                        </p>
                                    </div>
                                </div>
                        ...
                        >
                            <div class="bg-gray-50 p-4 rounded-lg shadow-sm">
                                <h3
                                    class="text-lg font-medium text-gray-900 border-b pb-2 mb-6 flex items-center"
                                >
                                    <i
                                        class="fas fa-images text-blue-500 mr-2"
                                    ></i>
                                    Galeria de Fotos da Unidade
                                    <span
                                        v-if="midiasReais?.length"
                                        class="ml-2 text-sm font-normal text-gray-600"
                                    >
                                        ({{ midiasReais.length }}
                                        {{
                                            midiasReais.length === 1
                                                ? "foto"
                                                : "fotos"
                                        }})
                                    </span>
                                ...
                                <div
                                    v-if="midiasReais && midiasReais.length > 0"
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            >
                                                ...
                                                <div
                                                    class="absolute top-3 left-3 bg-black bg-opacity-70 text-white text-xs px-3 py-1 rounded-full font-medium"
                                                >
                                                    {{
                                                        formatarNomeTipoMidia(
                                                            midia.midia_tipo
                                                                ?.nome ||
                                                                midia.midiaTipo
                                                                    ?.nome,
                                                ...
                                                <div
                                                    ...
                                                    "
                                                    class="absolute top-3 right-3 bg-blue-600 bg-opacity-90 text-white text-xs px-2 py-1 rounded-full"
                                                >
                                                    {{
                                                        midia.tamanho_formatado
                                                    }}
                                            ...
                                            >
                                                ...
                                                >
                                                    <i
                                                        class="fas fa-file text-gray-500 text-2xl"
                                                    ></i>
                                                </div>
                                                <p
                                                    class="text-sm text-gray-600 mb-4 font-medium"
                                                >
                                                    Arquivo
                                                </p>
                                ...
                                <div
                                    v-else
                                    class="bg-white p-12 rounded-xl shadow-sm text-center"
                                >
                                    <div class="max-w-sm mx-auto">
                                        ...
                                        >
                                            <i
                                                class="fas fa-photo-video text-gray-400 text-3xl"
                                            ></i>
                                        </div>
                                        <h4
                                        ...
                                        >
                                            Não há fotos ou arquivos cadastrados
                                            para esta unidade ainda.
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div
                                v-if="
                                    ambientesNaoPossui &&
                                    ambientesNaoPossui.length > 0
                                "
                                class="bg-gray-50 p-4 rounded-lg shadow-sm"
                            >
                                <h3
                                    class="text-lg font-medium text-gray-900 border-b pb-2 mb-4 flex items-center"
                                >
                                    <i
                                        class="fas fa-ban text-orange-500 mr-2"
                                    ></i>
                                    Ambientes que a Unidade Não Possui
                                    <span
                                        class="ml-2 text-sm font-normal text-gray-600"
                                        >({{
                                            ambientesNaoPossui.length
                                        }}
                                        ambiente{{
                                            ambientesNaoPossui.length === 1
                                                ? ""
                                                : "s"
                                        }})</span
                                    >
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            >
                                                <i
                                                    class="fas fa-home-slash text-orange-600 text-xl"
                                                ></i>
                                            </div>
                                        </div>
                                        ...
                                        <div class="text-center">
                                            ...
                                            <div
                                                ...
                                                "
                                                class="mt-2 text-xs text-gray-600 italic p-2 bg-gray-50 rounded"
                                            >
                                                "{{
                                                    ambiente.pivot.observacoes
                                                }}"
                                ...
                                <div
                                    class="mt-4 bg-orange-50 border border-orange-200 rounded-md p-3"
                                >
                                    <div class="flex items-start">
                                        <i
                                            class="fas fa-info-circle text-orange-500 mr-2 mt-0.5 flex-shrink-0"
                                        ></i>
                                        <div class="text-sm">
                                            <p
                                                class="font-medium text-orange-800 mb-1"
                                            >
                                                Informação:
                                            </p>
                                            <p class="text-orange-700">
                                                Foi marcado que "A unidade não
                                                possui este ambiente" durante o
                                                cadastro da unidade.
                                            </p>
                                        </div>
                                    </div>
                            ...
                            >
                                <div class="max-w-md mx-auto">
                                    ...
                                    >
                                        <i
                                            class="fas fa-photo-video text-gray-400 text-4xl"
                                        ></i>
                                    </div>
                                    <h4
                                    ...
                                    <p class="text-gray-600 leading-relaxed">
                                        Não há fotos cadastradas nem ambientes
                                        marcados como "não possui" para esta
                                        unidade.
                                    </p>
                                </div>
                            </div>
