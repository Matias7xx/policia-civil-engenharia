...
const deleteOrgao = () => {
    deleteForm.delete(
        route("admin.orgaos.destroy", orgaoBeingDeleted.value.id),
        {
            preserveScroll: true,
...
<template>
    <AppLayout title="Gerenciar Órgãos">
        <template #header>
            <div class="flex items-center space-x-4">
                ...
                <span class="text-white text-xl font-semibold pl-2"
                    >ENGENHARIA - Gerenciar Órgãos</span
                >
            </div>
        </template>
                                        ...
                                        >
                                            <thead class="bg-gray-50">
                                                <tr>
                                                    <th
                                                        scope="col"
                                                        class="w-2/3 px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider"
                                                    >
                                                        Nome
                                                    </th>
                                                    <th
                                                ...
                                                >
                                                    ...
                                                    >
                                                        <span
                                                            ...
                                                            "
                                                            class="text-base font-medium text-[#bea55a] hover:text-[#d4bf7a]"
                                                        >
                                                            {{ orgao.nome }}
                                                        </span>
                                                    </td>
                                                    ...
                                                    >
                                                        <Link
                                                            ...
                                                            "
                                                            class="text-indigo-600 hover:text-indigo-900 mr-4 flex items-center"
                                                        >
                                                            <PencilIcon
                                                                class="h-5 w-5 mr-1"
                                                            />
                        ...
                        <div class="mt-6" v-if="orgaos.data.length > 0">
                            <div class="flex items-center justify-between">
                                <div class="text-sm text-gray-700">
                                    Mostrando {{ orgaos.from }}-{{
                                        orgaos.to
                                    }}
                                    de {{ orgaos.total }} órgãos
                                </div>
        ...
        <DialogModal :show="confirmingOrgaoDeletion" @close="closeModal">
            <template #title> Excluir Órgão </template>

            <template #content>
                <div v-if="orgaoBeingDeleted">
                    Tem certeza que deseja excluir o órgão
                    <strong>{{ orgaoBeingDeleted.nome }}</strong
                    >?
