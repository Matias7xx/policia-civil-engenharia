...
const midiasReais = computed(() => {
    if (!props.midias || !Array.isArray(props.midias)) return [];
    return props.midias.filter((midia) => {
        if (midia.path === "nao_possui_ambiente") return false;
        if (midia.pivot && midia.pivot.nao_possui_ambiente === true) return false;
...
const ambientesNaoPossui = computed(() => {
    ...
    return props.midias
        .filter((midia) => {
            if (midia.path === "nao_possui_ambiente") return true;
            if (midia.pivot && midia.pivot.nao_possui_ambiente === true) return true;
            return false;
        })
        .map((midia) => {
            const nome = midia.midia_tipo?.nome || midia.midiaTipo?.nome || "Ambiente desconhecido";
            return { ...midia, nomeFormatado: formatarNomeTipoMidia(nome) };
        });
});
...
watch(
    ...
    (flash) => {
        ...
        if (flashMessage.value) setTimeout(() => (flashMessage.value = null), 5000);
    },
...
const isImage = (midia) => midia.is_imagem === true || midia.is_imagem === 1;
const openModal = (midia) => { selectedMedia.value = midia; isModalOpen.value = true; };
const closeModal = () => { isModalOpen.value = false; selectedMedia.value = null; };
                ...
                <div class="flex">
                    <div class="flex-shrink-0">
                        <i :class="flashMessage.includes('sucesso') || flashMessage.includes('salva')
                            ? 'fa-check-circle text-green-600 fas'
                            : 'fa-exclamation-circle text-amber-600 fas'"></i>
                    </div>
                    ...
                    <div class="p-4 sm:p-6 bg-white">
                        ...
                        <div v-if="unidade?.status" class="mb-6">
                            <div class="flex flex-wrap items-center gap-2">
                                ...
                                <span class="px-3 py-1 rounded-full text-xs sm:text-sm font-medium shadow-sm" :class="getStatusLabel.class">
                                    {{ getStatusLabel.text }}
                                </span>
                            </div>
                        ...
                        <div v-if="activeTab === 'dados-gerais'" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div class="bg-gray-50 p-4 rounded-lg shadow-sm">
                                ...
                                <dl class="grid grid-cols-1 gap-4">
                                    ...
                                    <div class="bg-white p-3 rounded-md shadow-sm hover:shadow-md transition-shadow">
                                        <dt class="font-medium text-gray-600 text-xs uppercase tracking-wider">Medidor de Água:</dt>
                                        <dd class="mt-1">{{ unidade?.numero_medidor_agua || "Não informado" }}</dd>
                                    </div>
                                    <div class="bg-white p-3 rounded-md shadow-sm hover:shadow-md transition-shadow">
                                        <dt class="font-medium text-gray-600 text-xs uppercase tracking-wider">Medidor de Energia:</dt>
                                        <dd class="mt-1">{{ unidade?.numero_medidor_energia || "Não informado" }}</dd>
                                    </div>
                                </dl>
                            </div>
                                        ...
                                        <dd class="mt-1">
                                            <span v-if="unidade?.latitude && unidade?.longitude" class="inline-flex items-center">
                                                ...
                                                <a :href="`https://www.google.com/maps?q=${unidade.latitude},${unidade.longitude}`"
                                                    target="_blank" class="ml-2 text-blue-500 hover:text-blue-700" title="Ver no Google Maps">
                            ...
                            <div v-if="unidade?.imovel_compartilhado_unidades" class="bg-gray-50 p-4 rounded-lg shadow-sm md:col-span-2">
                                ...
                                <div v-if="unidade.unidadesCompartilhadas && unidade.unidadesCompartilhadas.length > 0"
                                    ...
                                    <dd class="mt-1 flex flex-wrap gap-2">
                                        <span v-for="u in unidade.unidadesCompartilhadas" :key="u.id"
                                            class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800">
                                            {{ u.nome || "Sem nome" }}
                                        </span>
                                    </dd>
                                </div>
                                <div v-if="unidade.imovel_compartilhado_unidades_texto?.trim()"
                                    ...
                                    <dd class="mt-1 text-gray-900">{{ unidade.imovel_compartilhado_unidades_texto }}</dd>
                                </div>
                                <div v-if="(!unidade.unidadesCompartilhadas || unidade.unidadesCompartilhadas.length === 0) && !unidade.imovel_compartilhado_unidades_texto?.trim()"
                                    ...
                                    <dd class="mt-1 text-gray-600">Não informado</dd>
                                </div>
                            </div>
                            ...
                            <div v-if="unidade?.imovel_compartilhado_orgao" class="bg-gray-50 p-4 rounded-lg shadow-sm md:col-span-2">
                                ...
                                <div class="bg-white p-3 rounded-md shadow-sm hover:shadow-md transition-shadow">
                                    ...
                                    <dd class="mt-1 flex flex-wrap gap-2">
                                        <span v-if="!unidade.orgaosCompartilhados || unidade.orgaosCompartilhados.length === 0" class="text-gray-600">Não informado</span>
                                        <span v-else v-for="orgao in unidade.orgaosCompartilhados" :key="orgao.id"
                                            class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-blue-100 text-blue-800">
                                            {{ orgao.nome || "Sem nome" }}
                                        </span>
                                    </dd>
                        ...
                        <div v-if="activeTab === 'acessibilidade'" class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div class="bg-gray-50 p-4 rounded-lg shadow-sm col-span-full">
                                ...
                                <div v-if="acessibilidade" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                                    <!-- ★ Cada item tem suporte a "Não Possui" (null) -->
                        ...
                        <div v-if="activeTab === 'informacoes'" class="space-y-6">
                            ...
                            <div v-if="informacoes" class="bg-gray-50 p-4 rounded-lg shadow-sm">
                                ...
                                <div class="bg-white p-3 rounded-md shadow-sm hover:shadow-md transition-shadow mt-4 w-fit">
                                    <dt class="font-medium text-gray-600 text-xs uppercase tracking-wider">
                                        Ponto de energia próximo a algum ponto de água para a instalação de um purificador de água:
                                    </dt>
                            ...
                            <div v-if="informacoes" class="bg-gray-50 p-4 rounded-lg shadow-sm">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                                    <div v-for="(label, campo) in {
                                        qtd_recepcao: 'Recepções',
                                        qtd_wc_publico: 'WCs Públicos',
                                        qtd_gabinetes: 'Gabinetes',
                                        qtd_sala_oitiva: 'Salas de Oitiva',
                                        qtd_wc_servidores: 'WCs Servidores',
                                        qtd_alojamento_masculino: 'Alojamentos M',
                                        qtd_wc_alojamento_masculino: 'WCs Aloj. M',
                                        qtd_alojamento_feminino: 'Alojamentos F',
                                        qtd_wc_alojamento_feminino: 'WCs Aloj. F',
                                        qtd_xadrez_masculino: 'Xadrez Masculino',
                                        qtd_xadrez_feminino: 'Xadrez Feminino',
                                        qtd_sala_identificacao: 'Salas Identificação',
                                        qtd_cozinha: 'Cozinhas',
                                        qtd_area_servico: 'Áreas de Serviço',
                                        qtd_deposito_apreensao: 'Depósitos Apreensão'
                                    }" :key="campo"
                                        class="bg-white p-3 rounded-md shadow-sm hover:shadow-md transition-shadow">
                                        <dt class="font-medium text-gray-600 text-xs uppercase tracking-wider">{{ label }}:</dt>
                                        <dd class="mt-1">
                                            {{ informacoes[campo] ?? "Não informado" }}
                                        </dd>
                                    </div>
                                    <!-- Áreas dos xadrezes (sem "Não Possui") -->
                                    <div class="bg-white p-3 rounded-md shadow-sm hover:shadow-md transition-shadow">
                                        <dt class="font-medium text-gray-600 text-xs uppercase tracking-wider">Área Xadrez Masc. (m²):</dt>
                                        <dd class="mt-1">{{ informacoes.area_xadrez_masculino ? `${informacoes.area_xadrez_masculino} m²` : "Não informado" }}</dd>
                                    </div>
                                    <div class="bg-white p-3 rounded-md shadow-sm hover:shadow-md transition-shadow">
                                        <dt class="font-medium text-gray-600 text-xs uppercase tracking-wider">Área Xadrez Fem. (m²):</dt>
                            ...
                            <div v-if="informacoes" class="bg-gray-50 p-4 rounded-lg shadow-sm">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-4">
                                    <div v-for="(label, campo) in {
                                        tomadas_suficientes: 'Tomadas',
                                        luminarias_suficientes: 'Luminárias',
                                        pontos_rede_suficientes: 'Pontos de Rede',
                                        pontos_telefone_suficientes: 'Pontos de Telefone',
                                        pontos_ar_condicionado_suficientes: 'Pontos de A/C',
                                        pontos_hidraulicos_suficientes: 'Pontos Hidráulicos',
                                        pontos_sanitarios_suficientes: 'Pontos Sanitários'
                                    }" :key="campo"
                                        class="bg-white p-3 rounded-md shadow-sm hover:shadow-md transition-shadow">
                                        <dt class="font-medium text-gray-600 text-xs uppercase tracking-wider">{{ label }}:</dt>
                                        <dd class="mt-1 flex items-center">
                                            <i :class="`fas ${informacoes[campo] ? 'fa-check text-green-500' : 'fa-times text-red-500'} mr-2`"></i>
                                            {{ informacoes[campo] ? "Suficientes" : "Insuficientes" }}
                                        </dd>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div v-if="informacoes" class="bg-gray-50 p-4 rounded-lg shadow-sm">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                    <div v-for="(label, campo) in {
                                        extintor_po_quimico: 'Extintor Pó Químico',
                                        extintor_co2: 'Extintor CO2',
                                        extintor_agua: 'Extintor Água',
                                        placa_incendio: 'Placas de Emergência Para Incêndio'
                                    }" :key="campo"
                                        class="bg-white p-3 rounded-md shadow-sm hover:shadow-md transition-shadow">
                                        <dt class="font-medium text-gray-600 text-xs uppercase tracking-wider">{{ label }}:</dt>
                                        <dd class="mt-1">
...
{{ informacoes.escada_possui_corrimao || "Não informado" }}
                            ...
                            <div v-if="informacoes" class="bg-gray-50 p-4 rounded-lg shadow-sm">
                                ...
                                <div v-if="informacoes.tem_espaco_veiculos_apreendidos" class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                                    <div class="bg-white p-3 rounded-md shadow-sm hover:shadow-md transition-shadow">
                                        ...
                                        <dd class="mt-1 font-semibold text-lg">{{ informacoes.qtd_max_veiculos_automovel ? `${informacoes.qtd_max_veiculos_automovel} veículos` : "Não informado" }}</dd>
                                    </div>
                                    <div class="bg-white p-3 rounded-md shadow-sm hover:shadow-md transition-shadow">
                                        <dt class="font-medium text-gray-600 text-xs uppercase tracking-wider">O local apresenta segurança:</dt>
                                        <dd class="mt-1">
                                            <span class="inline-flex items-center px-2.5 py-2.5 rounded-full text-xs font-semibold"
                                                :class="{
                                                ...
                                                {{ informacoes.seguranca_local_veiculos === 'sim' ? 'Sim' : informacoes.seguranca_local_veiculos === 'nao' ? 'Não' : informacoes.seguranca_local_veiculos === 'parcial' ? 'Parcial' : 'Não informado' }}
                                            </span>
                                        </dd>
                                    </div>
                                    <div class="bg-white p-3 rounded-md shadow-sm hover:shadow-md transition-shadow">
                                    ...
                                    <div v-if="informacoes.observacoes_veiculos_apreendidos"
                                ...
                                <div v-else class="bg-yellow-50 p-3 rounded-md border border-yellow-200">
                                    <div class="flex items-center">
                                        ...
                                        <p class="text-yellow-800 text-sm">Esta unidade não possui espaço disponível para guardar veículos apreendidos.</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div v-if="activeTab === 'midias'" class="grid grid-cols-1 gap-6">
                            <div class="bg-gray-50 p-4 rounded-lg shadow-sm">
                                ...
                                <div v-if="midiasReais && midiasReais.length > 0"
                                    class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                                    <div v-for="midia in midiasReais" :key="midia.id"
                                        ...
                                        <div class="relative aspect-square bg-gray-100">
                                            <div v-if="isImage(midia)" class="relative h-full">
                                                <img :src="midia.url" @click="openModal(midia)"
                                                    ...
                                                    @error="$event.target.src = '/images/placeholder-image.jpg'" loading="lazy" />
                                                <div class="absolute inset-0 bg-black bg-opacity-0 group-hover:bg-opacity-60 transition-all duration-300 flex items-center justify-center opacity-0 group-hover:opacity-100">
                                                    <div class="flex space-x-3">
                                                        <button @click="openModal(midia)" class="flex items-center px-4 py-2 bg-white text-gray-800 rounded-lg shadow-lg hover:bg-gray-100 transition-colors">
                                                        ...
                                                        <a :href="midia.download_url || midia.url" download
                                            ...
                                            <div v-else class="flex flex-col items-center justify-center h-full p-6 text-center">
                                                ...
                                                <div class="flex space-x-2">
                                                    <a :href="midia.url" target="_blank" class="px-3 py-2 bg-blue-600 text-white rounded-lg text-xs hover:bg-blue-700 transition-colors">
                                                        <i class="fas fa-eye mr-1"></i>Ver
                                                    </a>
                                                    <a :href="midia.download_url || midia.url" download class="px-3 py-2 bg-green-600 text-white rounded-lg text-xs hover:bg-green-700 transition-colors">
                                        ...
                                        <div class="p-4">
                                            ...
                                            <div class="flex items-center justify-between text-xs text-gray-500">
                                                ...
                                                <span v-if="midia.created_at">{{ new Date(midia.created_at).toLocaleDateString("pt-BR") }}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div v-else class="bg-white p-12 rounded-xl shadow-sm text-center">
                                    <div class="max-w-sm mx-auto">
                                        ...
                                        <p class="text-gray-600 text-sm leading-relaxed">Não há fotos ou arquivos cadastrados para esta unidade ainda.</p>
                                    </div>
                                </div>
                            </div>

                            <!-- Ambientes Não Possui -->
                            <div v-if="ambientesNaoPossui && ambientesNaoPossui.length > 0" class="bg-gray-50 p-4 rounded-lg shadow-sm">
                                <h3 class="text-lg font-medium text-gray-900 border-b pb-2 mb-4 flex items-center">
                                    ...
                                    <span class="ml-2 text-sm font-normal text-gray-600">
                                        ({{ ambientesNaoPossui.length }} ambiente{{ ambientesNaoPossui.length === 1 ? "" : "s" }})
                                    </span>
                                </h3>
                                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                                    <div v-for="ambiente in ambientesNaoPossui" :key="ambiente.id"
                                        ...
                                        <div class="text-center">
                                            <h4 class="font-medium text-gray-900 text-sm mb-1">{{ ambiente.nomeFormatado }}</h4>
                                            <p class="text-xs text-gray-500 mb-2">Ambiente não disponível</p>
                                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium bg-orange-100 text-orange-800">
                                                <i class="fas fa-times mr-1"></i>Não possui
                                            </span>
                                ...
                                <div class="mt-4 bg-orange-50 border border-orange-200 rounded-md p-3">
                                    <div class="flex items-start">
                                        ...
                                        <p class="text-sm text-orange-700">Foi marcado que "A unidade não possui este ambiente" durante o cadastro.</p>
                                    </div>
                                </div>
                            </div>

                            <div v-if="(!midiasReais || midiasReais.length === 0) && (!ambientesNaoPossui || ambientesNaoPossui.length === 0)"
                                ...
                                <div class="max-w-md mx-auto">
                                    ...
                                    <p class="text-gray-600 leading-relaxed">Não há fotos cadastradas nem ambientes marcados como "não possui" para esta unidade.</p>
                                </div>
                            </div>
                        </div>
        ...
        <div v-if="isModalOpen" class="fixed inset-0 bg-black bg-opacity-75 flex items-center justify-center z-[1000]" @click.self="closeModal">
            <div class="bg-white rounded-lg p-4 max-w-3xl w-full mx-4 relative">
                ...
                <div class="flex flex-col items-center">
                    <img :src="selectedMedia?.url" alt="Mídia Ampliada" class="max-h-[70vh] max-w-full object-contain rounded-lg" />
                    <div class="mt-4 flex items-center justify-between w-full">
                        <h4 class="font-medium text-gray-900 truncate">{{ selectedMedia?.midia_tipo?.nome || "Arquivo" }}</h4>
...
@media (max-width: 640px) { .grid { grid-gap: 0.75rem; } .p-4 { padding: 0.75rem; } }
.modal-enter-active, .modal-leave-active { transition: opacity 0.3s ease; }
.modal-enter-from, .modal-leave-to { opacity: 0; }
.aspect-square { aspect-ratio: 1 / 1; }
.group:hover .group-hover\:scale-105 { transform: scale(1.05); }
.transition-all { transition-property: all; transition-timing-function: cubic-bezier(0.4, 0, 0.2, 1); }
.group-hover\:opacity-100 { transition: opacity 0.3s ease; }
.bg-opacity-70 { background-color: rgba(0, 0, 0, 0.7); }
.bg-opacity-90 { background-color: rgba(37, 99, 235, 0.9); }
</style>