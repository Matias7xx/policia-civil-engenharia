...
onMounted(async () => {
    ...
    try {
        if (props.midiaTipos && props.midiaTipos.length > 0) {
            ...
            midiaTipos.value = response.data
                .map((tipo) => {
                    const isAcessibilidadeAtiva =
                        props.acessibilidade &&
                        props.acessibilidade[tipo.nome] === true;
                    ...
                    return {
                        ...
                        shouldShow:
                            ![
                                ...
                                "elevador",
                                "sinalizacao_braile",
                            ].includes(tipo.nome) || isAcessibilidadeAtiva,
                    };
                })
        ...
        midiaTipos.value.forEach((tipo) => {
            if (tipo.isAreaInterna) {
                const temRegistroNaoPossui =
                    midiasAtuais.value?.some((midia) => {
                        const midiaTipoId =
                            midia.midia_tipo_id ||
                            midia.midia_tipo?.id ||
                            midia.midiaTipo?.id;
                        const isDummyRecord =
                            midia.path === "nao_possui_ambiente";
                        const hasPivotFlag =
                            midia.pivot &&
                            midia.pivot.nao_possui_ambiente === true;

                        return (
                            midiaTipoId === tipo.id &&
                            (isDummyRecord || hasPivotFlag)
                        );
                    }) || false;
        ...
        isLoading.value = false;
        toast.error(
            "Erro ao carregar tipos de mídia. Tente recarregar a página.",
        );
    }
});
...
watch(
    [() => midiasAtuais.value, () => midiaTipos.value],
    () => {
...
const refreshMidias = async () => {
    try {
        const response = await axios.get(
            `/unidades/${props.unidade.id}/midias`,
        );
...
const calculateBatchSize = (files) => {
    ...
    Object.entries(filesByType).forEach(([tipoId, typeFiles]) => {
        ...
        const typeSizeMB = typeFiles.reduce(
        ...
        if (typeSizeMB > MAX_BATCH_SIZE_MB || typeCount > BATCH_SIZE) {
            console.warn(
                `Tipo ${tipoId} tem ${typeCount} arquivos (${typeSizeMB.toFixed(1)}MB) que excede limites do lote. Será processado sozinho.`,
            );
    ...
    console.log(
        ...
        batches.map((batch, index) => ({
            ...
            tamanhoMB: batch
                .reduce((sum, f) => sum + f.file.size / (1024 * 1024), 0)
                .toFixed(1),
        })),
    );
...
const uploadInBatches = async (allFiles) => {
    ...
    toast.info(
        `Iniciando upload de ${allFiles.length} imagens em ${batches.length} lote(s)...`,
        { duration: 2000 },
    );
    ...
    try {
        for (let i = 0; i < batches.length; i++) {
            ...
            batchFormData.append(
                "total_files_in_batch",
                allFiles.length.toString(),
            );

            // Adicionar dados de "não possui ambiente" sempre
            Object.entries(ambientesNaoPossui.value).forEach(
                ([tipoId, naoPossui]) => {
                    batchFormData.append(
                        `ambientes_nao_possui[${tipoId}]`,
                        naoPossui ? "1" : "0",
                    );
                },
            );
            ...
            batch.forEach((fileData) => {
                ...
                batchFormData.append(
                    `midia_tipos[${fileIndex}]`,
                    fileData.midiaTipoId,
                );
            ...
            const response = await axios.post(
                ...
                batchFormData,
                {
                    ...
                    onUploadProgress: (progressEvent) => {
                        ...
                        uploadProgress.value.currentBatchProgress =
                            percentCompleted;

                        // Calcular progresso geral
                        const batchesCompleted = i;
                        const currentBatchContribution =
                            (percentCompleted / 100) *
                            (batch.length / allFiles.length);
                        const completedBatchesContribution =
                            uploadedCount / allFiles.length;
                        uploadProgress.value.overallProgress = Math.round(
                            (completedBatchesContribution +
                                currentBatchContribution) *
                                100,
                        );

                        // Calcular tempo estimado
                        const elapsed =
                            Date.now() - uploadProgress.value.startTime;
                        const rate =
                            uploadProgress.value.overallProgress / elapsed;
                        const remaining =
                            (100 - uploadProgress.value.overallProgress) / rate;
                        ...
                        if (remaining > 0 && isFinite(remaining)) {
                            ...
                            uploadProgress.value.estimatedTimeLeft =
                                minutes > 0
                                    ? `${minutes}m ${seconds}s`
                                    : `${seconds}s`;
                        }
                        ...
                        console.log(
                            `Lote ${i + 1}/${batches.length}: ${percentCompleted}% | Geral: ${uploadProgress.value.overallProgress}%`,
                        );
                    },
                },
            );
            ...
            if (response.status === 200 || response.status === 201) {
                ...
                throw new Error(
                    `Erro no lote ${i + 1}: ${response.data?.message || "Erro desconhecido"}`,
                );
            }
        }
        ...
        toast.success(
            `Sucesso! Todas as ${allFiles.length} imagens foram enviadas.`,
        );
        ...
        if (
            ...
            errorMsg =
                "Timeout: O envio demorou muito. Tente com menos imagens ou verifique sua conexão.";
        } else if (error.response?.status === 413) {
            errorMsg =
                "Erro 413: Arquivos muito grandes. Tente reduzir o tamanho das imagens ou envie menos por vez.";
...
const finalizarCadastro = async () => {
    try {
        ...
        const response = await axios.post(route("midias.store"), formData, {
            headers: {
...
const validateQuantityPerType = () => {
    ...
    midiaTipos.value.forEach((tipo) => {
        ...
        const removedCount = midiasToRemove.value.filter((removedId) => {
            return midiasPorTipo.value[tipo.id]?.some(
                (midia) => midia.id === removedId,
            );
        }).length;
        ...
        if (finalCount > 0) {
            if (finalCount < 2) {
                errors.push(
                    `${formatarNomeTipoMidia(tipo.nome)}: mínimo de 2 fotos (atual: ${finalCount})`,
                );
            }
            if (finalCount > 3) {
                errors.push(
                    `${formatarNomeTipoMidia(tipo.nome)}: máximo de 3 fotos (atual: ${finalCount})`,
                );
            }
        }
    });
...
const getPhotoCount = (midiaTipoId) => {
    const existingCount =
        midiasPorTipo.value[midiaTipoId]?.filter(
            (midia) => !midiasToRemove.value.includes(midia.id),
        ).length || 0;
...
const openFileInput = (midiaTipoId) => {
    if (
        !ambientesNaoPossui.value[midiaTipoId] &&
...
const handleFileChange = (midiaTipoId, event) => {
    ...
    if (totalCount > 3) {
        alert(
            `Máximo de 3 fotos permitidas por tipo de mídia. Você pode adicionar mais ${3 - currentCount} foto(s).`,
        );
        event.target.value = "";
        return;
    }
...
const saveOrFinalizeMidias = () => {
    ...
    requiredTypeIds.forEach((tipoId) => {
        const tipoName =
            midiaTipos.value.find((t) => t.id === tipoId)?.nome ||
            `Tipo ${tipoId}`;
        ...
        const isMissing =
            (!existingMediaNotRemoved || willBeReplaced) && !hasNewMedia;
    ...
    if (!willUseBatches || isFinalizing) {
        if (
            ...
            missingAcessibilidadeTypes.length > 0 ||
            missingAreaInternaTypes.length > 0
        ) {
            let errorMessage = "";
    ...
    const hasNaoPossuiChanges = Object.values(ambientesNaoPossui.value).some(
        (v) => v,
    );

    if (!hasFiles && !hasRemovals && !hasNaoPossuiChanges) {
        const errorMsg =
            "Por favor, faça pelo menos uma alteração antes de salvar.";
    ...
    if (allFiles.length > MAX_TOTAL_FILES) {
        toast.warning(
            `Você selecionou ${allFiles.length} imagens. O sistema suporta até ${MAX_TOTAL_FILES} fotos (43 campos × 3 fotos cada).\n\nSe necessário, você pode enviar em etapas: primeiro Área Externa, depois Área Interna, depois Acessibilidade.`,
        );
        return;
    }
    ...
    if (totalSizeMB > maxTotalSizeMB) {
        toast.warning(
            `O tamanho total das imagens (${Math.round(totalSizeMB)}MB) excede o limite recomendado de ${maxTotalSizeMB}MB.\n\nTente comprimir as imagens ou enviar em etapas menores.`,
        );
        return;
    }
    ...
    axios
        ...
        .then(async (response) => {
            ...
            if (response.status === 200 || response.status === 201) {
                const successMsg =
                    response.data.message || "Mídias salvas com sucesso!";

                // Verificar se há redirecionamento (finalização)
                if (response.data.redirect) {
                    toast.success(
                        "Cadastro finalizado com sucesso! Redirecionando...",
                    );
                ...
                const errorMsg =
                    ...
                    "Erro inesperado ao salvar as mídias.";
                toast.error(errorMsg);
                emit("saved", errorMsg);
            }
        })
        .catch((error) => {
            ...
            if (
                ...
                toast.validationErrors(error.response.data.errors, {
                    showAll: false,
                });
                return;
            } else if (error.response?.data?.message) {
...
<template>
    <FormSection @submitted="saveOrFinalizeMidias">
        <template #title> Fotos da Unidade </template>

        <template #description>
            <p class="mt-2 text-sm text-gray-600">
                Formatos aceitos: JPG, PNG (máx. 10MB por foto) -
                <strong class="text-blue-600"
                    >No mínimo 2 e no máximo 3 fotos por tipo</strong
                >
            </p>
            <p class="mt-2 text-sm text-blue-600">
                <strong>Para Área Interna:</strong> Inclua fotos OU marque "Não
                possui" se a unidade não tem esse ambiente.
            </p>
            <p class="mt-1 text-sm text-blue-600">
                <strong>Para Acessibilidade:</strong> É obrigatório incluir
                fotos dos recursos marcados no formulário de acessibilidade.
            </p>
        </template>
            ...
            <div class="col-span-6">
                ...
                <div v-else-if="midiaTipos.length === 0" class="mt-2">
                    <p class="text-red-600">
                        Nenhum tipo de mídia disponível ou erro ao carregar.
                    </p>
                </div>

                <div v-else>
                    <div
                        v-for="(grupo, index) in getTabelas"
                        :key="index"
                        :id="`section-${index}`"
                        class="mb-10"
                    >
                        <div
                            class="flex items-center justify-between mb-6 pb-4 border-b-2 border-gray-100"
                        >
                            <h3
                            ...
                            >
                                <div
                                    ...
                                    :class="{
                                        'bg-indigo-100 text-indigo-600':
                                            grupo[0] === 'Área Externa',
                                        'bg-green-100 text-green-600':
                                            grupo[0] === 'Área Interna',
                                        'bg-blue-100 text-blue-600':
                                            grupo[0] === 'Acessibilidade',
                                    }"
                                >
                                    <svg
                                        v-if="grupo[0] === 'Área Externa'"
                                        class="w-4 h-4"
                                        fill="none"
                                        stroke="currentColor"
                                        viewBox="0 0 24 24"
                                    ...
                                    <svg
                                        v-else-if="grupo[0] === 'Área Interna'"
                                        class="w-4 h-4"
                                        fill="none"
                                        stroke="currentColor"
                                        viewBox="0 0 24 24"
                                    ...
                                    >
                                        <path
                                            d="M12 2C13.1 2 14 2.9 14 4S13.1 6 12 6 10 5.1 10 4 10.9 2 12 2M21 9V7L15 7.5V9M15 9.5L13.5 7H10V8.5H11.5L13 11L11.5 12.5L10.5 11.5L9.5 12.5L12 15L15 12L13.5 10.5L15 9.5M7.5 22C5.6 22 4 20.4 4 18.5S5.6 15 7.5 15 11 16.6 11 18.5 9.4 22 7.5 22M7.5 17C6.7 17 6 17.7 6 18.5S6.7 20 7.5 20 9 19.3 9 18.5 8.3 17 7.5 17M18.5 22C16.6 22 15 20.4 15 18.5S16.6 15 18.5 15 22 16.6 22 18.5 20.4 22 18.5 22M18.5 17C17.7 17 17 17.7 17 18.5S17.7 20 18.5 20 20 19.3 20 18.5 19.3 17 18.5 17Z"
                                        />
                                    </svg>
                                </div>
                                {{ grupo[0] }}
                                <span
                                    v-if="grupo[0] === 'Área Externa'"
                                    class="text-red-500 ml-2 text-lg"
                                    >*</span
                                >
                            </h3>
                            ...
                            <div class="text-sm text-gray-500 font-medium">
                                {{ grupo[1].length }}
                                {{ grupo[1].length === 1 ? "tipo" : "tipos" }}
                            </div>
                        </div>

                        <div
                            class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-3 gap-5"
                        >
                            <div
                                v-for="midiaTipo in grupo[1]"
                                :key="midiaTipo.id"
                            ...
                            >
                                <div
                                    class="px-6 py-4 bg-gray-50 border-b border-gray-100 min-h-[100px]"
                                >
                                    <div
                                        class="flex items-start justify-between h-full"
                                    >
                                        <div
                                            class="flex-1 min-w-0 mr-3 flex flex-col justify-between h-full"
                                        >
                                            ...
                                            >
                                                <h4
                                                    class="text-sm font-semibold text-gray-900 flex items-start leading-snug"
                                                >
                                                    <span
                                                    ...
                                                    <span
                                                        v-if="
                                                            midiaTipo.nome.includes(
                                                                'foto_',
                                                            ) &&
                                                            !midiaTipo.isAreaInterna
                                                        "
                                                        class="text-red-500 ml-1 font-bold flex-shrink-0"
                                                        >*</span
                                                    >
                                                    <span
                                                        v-if="
                                                            midiaTipo.isAcessibilidadeObrigatoria
                                                        "
                                                        class="text-red-500 ml-1 font-bold flex-shrink-0"
                                                        >*</span
                                                    >
                                            ...
                                            >
                                                <span
                                                    class="text-xs font-medium text-gray-600 bg-gray-100 px-2 py-1 rounded-full whitespace-nowrap flex items-center"
                                                >
                                                    {{
                                                        getPhotoCount(
                                                            midiaTipo.id,
                                                ...
                                                >
                                                    <label
                                                        class="flex items-center text-xs font-medium text-gray-600 cursor-pointer whitespace-nowrap"
                                                    >
                                                        <input
                                                            type="checkbox"
                                                            :checked="
                                ...
                                <div class="p-6">
                                    ...
                                    >
                                        ...
                                        >
                                            <div class="mb-3">
                                                <svg
                                                    class="w-12 h-12 text-gray-400 group-hover:text-blue-500 transition-colors duration-200"
                                                    :class="{
                                    ...
                                    <div
                                        v-if="
                                            previewImages[midiaTipo.id]?.length
                                        "
                                        class="mt-6"
                                    >
                                        <div
                                        ...
                                        >
                                            ...
                                            >
                                                <div
                                                    class="w-5 h-5 bg-blue-100 rounded-full flex items-center justify-center mr-2"
                                                >
                                                    <svg
                                    ...
                                    <div
                                        v-if="
                                            midiasPorTipo[midiaTipo.id]?.length
                                        "
                                        class="mt-6"
                                    >
                                        <h5
                                        ...
                                        >
                                            <div
                                                class="w-5 h-5 bg-green-100 rounded-full flex items-center justify-center mr-2"
                                            >
                                                <svg
                                        ...
                                        <div class="grid grid-cols-3 gap-3">
                                            ...
                                            >
                                                ...
                                                <div
                                                    class="absolute bottom-2 left-2 bg-black bg-opacity-75 text-white text-xs px-2 py-1 rounded backdrop-blur-sm"
                                                >
                                                    {{
                                                        getFileSize(
                                                            midia.tamanho,
        ...
        <template #actions>
            <div class="flex items-center justify-between">
                ...
                >
                    ...
                    <span v-else-if="form.processing"> Salvando... </span>
                    <span v-else>
                        {{
        ...
        >
            ...
            >
                <div>
                    ...
                    >
                        <svg
                            class="h-6 w-6 text-blue-600 animate-pulse"
                            fill="none"
                            stroke="currentColor"
                            viewBox="0 0 24 24"
                        >
                    ...
                    <div class="text-center">
                        ...
                        <p class="text-sm text-gray-500 mb-4">
                            Por favor, aguarde enquanto suas fotos são
                            processadas...
                        </p>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        >
                            <span
                                >Enviando {{ uploadProgress.currentBatch }} de
                                {{ uploadProgress.totalBatches }}</span
                            >
                            <span
                                >{{
                                    uploadProgress.currentBatchProgress
                                }}%</span
                            >
                        </div>
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        ...
                        <div class="bg-gray-50 rounded-lg p-3 text-center">
                            ...
                            <div class="text-xs text-gray-600">
                                de {{ uploadProgress.totalFiles }} arquivos
                            </div>
                        </div>
                        ...
                        <div class="bg-gray-50 rounded-lg p-3 text-center">
                            <div class="text-2xl font-bold text-gray-900">
                                {{ uploadProgress.estimatedTimeLeft }}
                            </div>
                            <div class="text-xs text-gray-600">
                                tempo restante
                            </div>
                    ...
                    <div class="text-center">
                        <p class="text-sm text-gray-600">
                            <span v-if="uploadProgress.totalBatches > 1">
                                Processando lote
                                {{ uploadProgress.currentBatch }} de
