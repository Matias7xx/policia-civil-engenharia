...
const persistentState = ref({
    imovel_compartilhado_orgao:
        props.unidade?.imovel_compartilhado_orgao || false,
    imovel_compartilhado_orgao_ids: [],
    imovel_compartilhado_unidades:
        props.unidade?.imovel_compartilhado_unidades || false,
    imovel_compartilhado_unidades_ids: [],
    imovel_compartilhado_unidades_texto:
        props.unidade?.imovel_compartilhado_unidades_texto || "",
});
...
const initializeUnidadeIds = () => {
    if (props.unidade?.unidadesCompartilhadas?.length) {
        persistentState.value.imovel_compartilhado_unidades_ids =
            props.unidade.unidadesCompartilhadas.map((unidade) =>
                Number(unidade.id),
            );
    }
};
...
const filteredUnidades = computed(() => {
    ...
    return (props.unidades || []).filter((unidade) =>
        unidade.nome
            .toLowerCase()
            .includes(unidadeSearchTerm.value.toLowerCase()),
    );
});

const selectedOrgaos = computed(() => {
    return (props.orgaos || []).filter((orgao) =>
        persistentState.value.imovel_compartilhado_orgao_ids.includes(
            Number(orgao.id),
        ),
    );
});
...
const selectedUnidades = computed(() => {
    return (props.unidades || []).filter((unidade) =>
        persistentState.value.imovel_compartilhado_unidades_ids.includes(
            Number(unidade.id),
        ),
    );
});
...
const methods = {
    ...
    removeOrgao: (orgaoId) => {
        const numericId = Number(orgaoId);
        persistentState.value.imovel_compartilhado_orgao_ids =
            persistentState.value.imovel_compartilhado_orgao_ids.filter(
                (id) => id !== numericId,
            );
    },

    isOrgaoSelected: (orgaoId) => {
        return persistentState.value.imovel_compartilhado_orgao_ids.includes(
            Number(orgaoId),
        );
    },
    ...
    removeUnidade: (unidadeId) => {
        const numericId = Number(unidadeId);
        persistentState.value.imovel_compartilhado_unidades_ids =
            persistentState.value.imovel_compartilhado_unidades_ids.filter(
                (id) => id !== numericId,
            );
    },

    isUnidadeSelected: (unidadeId) => {
        return persistentState.value.imovel_compartilhado_unidades_ids.includes(
            Number(unidadeId),
        );
    },
};
    ...
    (newUnidade) => {
        if (newUnidade) {
            ...
            persistentState.value.imovel_compartilhado_orgao =
                newUnidade.imovel_compartilhado_orgao || false;
            persistentState.value.imovel_compartilhado_unidades =
                newUnidade.imovel_compartilhado_unidades || false;
            persistentState.value.imovel_compartilhado_unidades_texto =
                newUnidade.imovel_compartilhado_unidades_texto || "";
        }
    },
    { deep: true },
);
...
const handleClickOutside = (event) => {
    ...
    if (
        unidadeDropdownRef.value &&
        !unidadeDropdownRef.value.contains(event.target)
    ) {
...
const saveDadosGerais = () => {
    ...
    form.imovel_compartilhado_unidades =
        persistentState.value.imovel_compartilhado_unidades;
    form.imovel_compartilhado_unidades_ids = [
        ...persistentState.value.imovel_compartilhado_unidades_ids,
    ];
    ...
    if (
        persistentState.value.imovel_compartilhado_orgao &&
        persistentState.value.imovel_compartilhado_orgao_ids.length === 0
    ) {
    ...
    if (
        ...
        errors.push(
            "Selecione unidades ou descreva quais unidades compartilham o imóvel.",
        );
        toast.error(
            "Selecione unidades ou descreva quais unidades compartilham o imóvel.",
        );
    ...
    // Limpar telefones
    form.telefone_1 = form.telefone_1
        ? form.telefone_1.replace(/[^0-9]/g, "")
        : "";
    form.telefone_2 = form.telefone_2
        ? form.telefone_2.replace(/[^0-9]/g, "")
        : "";
    ...
    form.post(route("unidades.saveDadosGerais"), {
        ...
        onError: (errors) => {
            ...
            emit(
                "saved",
                "Erro ao salvar os dados. Verifique os campos.",
                null,
            );
        },
    });
            ...
            <div class="p-6">
                ...
                >
                    <div>
                        ...
                        <InputError
                            :message="form.errors.tipo_judicial"
                            class="mt-1"
                        />
                    </div>
                    <div>
                    ...
                    <div class="md:col-span-3">
                        <div class="flex items-center mt-2">
                            <Checkbox
                                ...
                                v-model:checked="
                                    persistentState.imovel_compartilhado_unidades
                                "
                                :disabled="
                                    !isEditable || !permissions?.canUpdateTeam
                                "
                            />
                            <InputLabel
                                for="imovel_compartilhado_unidades"
                                value="Imóvel compartilhado com outra(s) Unidades Policiais?"
                                class="ml-2"
                            />
                        </div>
                        <InputError
                            :message="form.errors.imovel_compartilhado_unidades"
                            class="mt-1"
                        />
                    </div>
                    ...
                    <!-- Seletor de unidades -->
                    <div
                        v-if="persistentState.imovel_compartilhado_unidades"
                        class="md:col-span-3"
                    >
                        <InputLabel
                            for="imovel_compartilhado_unidades_ids"
                            value="Unidades Compartilhadas"
                            ...
                            <div class="flex flex-wrap gap-2">
                                ...
                                >
                                    ...
                                    <button
                                        ...
                                        type="button"
                                        @click="
                                            methods.removeUnidade(unidade.id)
                                        "
                        ...
                        <div class="relative" ref="unidadeDropdownRef">
                            <button
                                type="button"
                                @click="
                                    showUnidadeDropdown = !showUnidadeDropdown
                                "
                                :disabled="
                                    !isEditable || !permissions?.canUpdateTeam
                                "
                        ...
                        <InputError
                            :message="
                                form.errors.imovel_compartilhado_unidades_ids
                            "
                            class="mt-1"
                        />
                        ...
                        <div class="mt-4">
                            <InputLabel
                                for="imovel_compartilhado_unidades_texto"
                                value="Ou descreva quais unidades compartilham o imóvel"
                                class="text-sm font-medium text-gray-700"
                            />
                            <TextInput
                                id="imovel_compartilhado_unidades_texto"
                                v-model="
                                    persistentState.imovel_compartilhado_unidades_texto
                                "
                                type="text"
                                class="mt-1 block w-full"
                                :disabled="
                            ...
                            <p class="mt-1 text-xs text-gray-500">
                                Você pode selecionar unidades acima ou descrever
                    ...
                    <div class="md:col-span-3">
                        <div class="flex items-center mt-2">
                            <Checkbox
                                ...
                                v-model:checked="
                                    persistentState.imovel_compartilhado_orgao
                                "
                                :disabled="
                                    !isEditable || !permissions?.canUpdateTeam
                                "
                            />
                            <InputLabel
                                for="imovel_compartilhado_orgao"
                                value="Imóvel compartilhado com outro(s) órgão(s)"
                                class="ml-2"
                            />
                        </div>
                        <InputError
                            :message="form.errors.imovel_compartilhado_orgao"
                            class="mt-1"
                        />
                    </div>
                    ...
                    <!-- Seletor de órgãos -->
                    <div
                        v-if="persistentState.imovel_compartilhado_orgao"
                        class="md:col-span-3"
                    >
                        <InputLabel
                            for="imovel_compartilhado_orgao_ids"
                            value="Órgãos Compartilhados *"
                            class="text-sm font-medium text-gray-700 mb-2"
                        />
                                ...
                                <span class="text-gray-700">
                                    {{
                                        selectedOrgaos.length === 0
                                            ? "Selecione os órgãos..."
                                            : `${selectedOrgaos.length} órgão(s) selecionado(s)`
                                    }}
                                </span>
                                <svg
                                    class="w-5 h-5 text-gray-400"
                        ...
                        <InputError
                            :message="
                                form.errors.imovel_compartilhado_orgao_ids
                            "
                            class="mt-1"
                        />
                    </div>
